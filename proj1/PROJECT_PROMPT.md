...
| 항목 | URL / 설명 |
|------|-----------|
| 기자 목록 | `https://media.naver.com/journalists/whole?officeId={officeId}` |
| officeId 매핑 | `MEDIA_OFFICE_ID_MAP`에 정의된 51개 매체 (`연합뉴스=001` ~ `SBS Biz=096`) |
...
### Step 1 — 매체명 기반 officeId 확인
- 입력 매체명을 고정 매핑 테이블에서 조회
- 지원 매체는 `MEDIA_OFFICE_ID_MAP`에 정의된 51개 매체
- 매핑된 `officeId`로 `https://media.naver.com/journalists/whole?officeId={officeId}`에 바로 이동
...
### 호출 2 — 토킹포인트 (최신 기사 20건 + 포스코 기사 5건 입력)
기자의 최신 관심사와 포스코 관련 보도를 함께 기준으로 토킹포인트 생성.
반환 JSON 구조:
```json
{
  "posco_coverage": {
...
- 각 `content`: 포스코 홍보 담당자가 기자에게 직접 말하는 1인칭 대화 시나리오, 공백 포함 **100자 이내**
...
| 5차 | 매체 탐색을 `001~099` 순회에서 매체명 기반 고정 officeId 매핑으로 변경 |

---

## 12. 향후 개선 고려사항

- 매체명-officeId 매핑 테이블 확장 및 관리 방식 개선
