...
class NaverJournalistCrawler:
    ...
    async def _async_search(self, media_name: str, journalist_name: str) -> Dict:
        # 1단계: 입력 매체명으로 officeId 바로 확인
        self._update(f"'{media_name}' 매체 정보 확인 중…", 5)
        office_id = self._resolve_office_id(media_name)
        if not office_id:
            supported = ", ".join(self.MEDIA_OFFICE_ID_MAP.keys())
            return {"error": f"'{media_name}' 매체는 지원하지 않습니다. 지원 매체: {supported}"}
        ...
        async with async_playwright() as p:
            ...
            try:
                # 2단계: 해당 officeId 기자 URL 탐색 (스크롤 10회)
    ...
    def _resolve_office_id(self, media_name: str) -> Optional[str]:
        normalized_media_name = re.sub(r"\s+", "", media_name or "").lower()
        for supported_media_name, office_id in self.MEDIA_OFFICE_ID_MAP.items():
            normalized_supported_name = re.sub(r"\s+", "", supported_media_name).lower()
            if normalized_media_name == normalized_supported_name:
                return office_id
        return None
